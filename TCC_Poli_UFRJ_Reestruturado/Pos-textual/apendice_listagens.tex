...
\paragraph{}O módulo \texttt{common.py}, compartilhado pelos solvers de escoamento 2D, concentra a geração da malha triangular e a montagem das matrizes globais de rigidez e massa do MEF --- o núcleo numérico reutilizado pelos módulos de escoamento potencial e de Navier-Stokes.

\lstinputlisting[
  caption={Núcleo MEF 2D compartilhado --- \texttt{common.py} (completo)},
  label={lst:common2d}
]{../simulations/mef/escoamento-fluido-2d/common.py}

\paragraph{}Do solver de Navier-Stokes 2D, reproduz-se o trecho central: o laço de avanço temporal da formulação Vorticidade-Corrente, com a solução da equação de Poisson para $\psi$, a imposição da condição de contorno de Thom nas paredes, a recuperação do campo de velocidades e o transporte semi-implícito da vorticidade.
% Trecho extraído de simulations/mef/escoamento-fluido-2d/navier-stokes/simulation.py,
% função solve_navier_stokes, laço "while current_step < total_steps" (linhas 225-276).
% Os simulation.py não devem ser editados (geraram as figuras do Cap. de Resultados),
% portanto o linerange abaixo é estável.
